...
public class ImageUtils
{
     ...
     *  RoundedCornersTransformation.CornerType.all 设置圆角的位置
     *
     */
    ...
    public static void loadCirCleImage(Context context, String url, int errorResource, ImageView imageView)
    {
        ...
        Glide.with(context)                      //可以是context activity fragment FragmentActivity
                ...
                .diskCacheStrategy(DiskCacheStrategy.RESULT)       //缓存多种尺寸
                .skipMemoryCache(false)          //头像不跳过内存缓存，因为每次刷新都会闪烁（显示默认再显示网络）
                .into(imageView);               //需要加载的控件
    }
     ...
     * 没有圆角  设置缓存类型
     */
    public static void loadImage(Context context, String url, int beforResource, int errorResource, float thumbnail, DiskCacheStrategy diskCacheStrategy, ImageView imageView)
    {
        if (context == null)
            return;
        Glide.with(context)                      //可以是context activity fragment FragmentActivity
                ...
                .error(errorResource)            //加载失败的图片
                .placeholder(beforResource)      //加载中的图片
                .diskCacheStrategy(diskCacheStrategy)       //缓存多种尺寸
    ...
    {
        loadImage(context, url, beforResource, errorResource, 1f, DiskCacheStrategy.RESULT, imageView);
    }
