...
//                        setImageEvent(viewHolder, uuid,context);
    ...
    public String getSummary() {
        return MyApplication.getInstance().getApplicationContext().getString(R.string.summary_image);
    }
                ...
                image.getImage(new TIMValueCallBack<byte[]>() {
                    ...
                    public void onSuccess(byte[] bytes) {
                        if (FileUtil.isFileExist(uuid+".jpg", Environment.DIRECTORY_DOWNLOADS)) {
                            Toast.makeText(MyApplication.getInstance().getApplicationContext(), MyApplication.getInstance().getApplicationContext().getString(R.string.save_exist), Toast.LENGTH_SHORT).show();
                            return;
                        }
                        File mFile = FileUtil.createFile(bytes, uuid+".jpg", Environment.DIRECTORY_DOWNLOADS);
                        if (mFile != null) {
                            Toast.makeText(MyApplication.getInstance().getApplicationContext(), MyApplication.getInstance().getApplicationContext().getString(R.string.save_succ) +
                                    "path : " + mFile.getAbsolutePath(), Toast.LENGTH_SHORT).show();
                        } else {
                            Toast.makeText(MyApplication.getInstance().getApplicationContext(), MyApplication.getInstance().getApplicationContext().getString(R.string.save_fail), Toast.LENGTH_SHORT).show();
                        }
                    }
                });
            }
            ...
            if (!isDownloading){
                ...
                image.getImage(FileUtil.getCacheFilePath(image.getUuid()), new TIMCallBack() {
                    ...
                    public void onError(int i, String s) {
                        ...
                        Toast.makeText(context, MyApplication.getInstance().getApplicationContext().getString(R.string.download_fail), Toast.LENGTH_SHORT).show();
                        isDownloading = false;
                    }
...
//                        context.startActivity(intent);
                ...
                Toast.makeText(context, MyApplication.getInstance().getApplicationContext().getString(R.string.downloading), Toast.LENGTH_SHORT).show();
            }
        }
    }
}
