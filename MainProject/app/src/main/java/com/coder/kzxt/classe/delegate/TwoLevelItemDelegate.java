...
public class TwoLevelItemDelegate extends TreeParentItem<OrganizationBean.TwoLevelBean> implements TreeItem.onClickChangeListener {

    private OrganizationActivity mContext;

    public TwoLevelItemDelegate(OrganizationBean.TwoLevelBean data, TreeParentItem parentItem) {
        super(data, parentItem);
    }

    @Override
    protected List<TreeItem> initChildsList(OrganizationBean.TwoLevelBean data) {
        ArrayList<TreeItem> treeItems = new ArrayList<>();
        ArrayList<OrganizationBean.ThreeLevelBean> childrens = data.getChildren();
        if (null == childrens) {
            return null;
        }
        if (childrens.size() > 0) {
        ...
        for (int i = 0; i < size; i++) {
            ThreeLevelItemDelegate threeItem = new ThreeLevelItemDelegate(childrens.get(i),this);
