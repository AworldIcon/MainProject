...
public class NineLevelItemDelegate extends TreeParentItem<OrganizationBean.NineLevelBean> implements TreeItem.onClickChangeListener {

    private OrganizationActivity mContext;

    public NineLevelItemDelegate(OrganizationBean.NineLevelBean data, TreeParentItem parentItem) {
        super(data, parentItem);
    }

    @Override
    protected List<TreeItem> initChildsList(OrganizationBean.NineLevelBean data) {
        ArrayList<TreeItem> treeItems = new ArrayList<>();
        ArrayList<OrganizationBean.TenLevelBean> childrens = data.getChildren();
        if (null == childrens) {
            return null;
        }
        if (childrens.size() > 0) {
        ...
        for (int i = 0; i < size; i++) {
            TenLevelItemDelegate tenItem = new TenLevelItemDelegate(childrens.get(i),this);
            treeItems.add(tenItem);
        }
        return treeItems;
    }
