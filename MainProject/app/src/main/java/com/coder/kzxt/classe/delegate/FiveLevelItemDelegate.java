...
public class FiveLevelItemDelegate extends TreeParentItem<OrganizationBean.FiveLevelBean> implements TreeItem.onClickChangeListener {

    private OrganizationActivity mContext;

    public FiveLevelItemDelegate(OrganizationBean.FiveLevelBean data, TreeParentItem parentItem) {
        super(data, parentItem);
    }

    @Override
    protected List<TreeItem> initChildsList(OrganizationBean.FiveLevelBean data) {
        ArrayList<TreeItem> treeItems = new ArrayList<>();
        ArrayList<OrganizationBean.SixLevelBean> childrens = data.getChildren();
        if (null == childrens) {
            return null;
        }
        if (childrens.size() > 0) {
        ...
        for (int i = 0; i < size; i++) {
            SixLevelItemDelegate sixItem = new SixLevelItemDelegate(childrens.get(i),this);
            treeItems.add(sixItem);
        }
        return treeItems;
    }
