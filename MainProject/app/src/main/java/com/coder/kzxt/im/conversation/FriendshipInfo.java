...
public class FriendshipInfo extends Observable implements Observer
{
    ...
    /**
     * 添加好友
     * @param identifier 添加人的id
     */
    public void addFriend(String identifier)
    {
        if (isFriend(identifier))
        {
            return;
        }
        ...
        List<TIMAddFriendRequest> timAddFriendRequests = new ArrayList<TIMAddFriendRequest>();
        timAddFriendRequests.add(timAddFriendRequest);
        TIMFriendshipManager.getInstance().addFriend(timAddFriendRequests, new TIMValueCallBack<List<TIMFriendResult>>()
        {
            @Override
