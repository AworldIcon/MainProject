 ...
 * 班级话题fragment
 */
public class ClassTopicFragment extends BaseFragment implements SwipeRefreshLayout.OnRefreshListener,HttpCallBack,OnClassTopicInterface{
...
//        myPullRecyclerView.setLayoutManager(new LinearLayoutManager(mContext));
    ...
    private void getClassTopic(int index) {
        HttpGetBuilder builder = new HttpGetBuilder(mContext);
        if (index == 2) {
            builder.setUrl(UrlsNew.GET_GROUP_TOPIC_COLLECT);
        } else {
