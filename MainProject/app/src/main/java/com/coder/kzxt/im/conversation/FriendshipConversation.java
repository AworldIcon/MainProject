...
public class FriendshipConversation extends Conversation {
    ...
    public String getAvatar() {
        FriendProfile profile = FriendshipInfo.getInstance().getProfile(lastMessage.getIdentifier());
    ...
    public String getLastMessageSummary() {
        ...
        switch (lastMessage.getType()){
            case TIM_FUTURE_FRIEND_PENDENCY_IN_TYPE://我收到的好友申请的未决消息
                return name + MyApplication.getInstance().getApplicationContext().getString(R.string.summary_friend_add);
            case TIM_FUTURE_FRIEND_PENDENCY_OUT_TYPE://我发出的好友申请的未决消息
                return MyApplication.getInstance().getApplicationContext().getString(R.string.summary_me) + MyApplication.getInstance().getApplicationContext().getString(R.string.summary_friend_add_me) + name;
            case TIM_FUTURE_FRIEND_DECIDE_TYPE://已决消息
                return MyApplication.getInstance().getApplicationContext().getString(R.string.summary_friend_added) + name;
            case TIM_FUTURE_FRIEND_RECOMMEND_TYPE://好友推荐
                return MyApplication.getInstance().getApplicationContext().getString(R.string.summary_friend_recommend) + name;
            default:
                return "";
        }
    }
    ...
    public String getName() {
        return MyApplication.getInstance().getApplicationContext().getString(R.string.conversation_system_friend);
